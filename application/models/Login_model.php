...
class login_model extends CI_Model
{
    ...
    function check_login($userid, $password)
    {
       $query = $this->db->query("SELECT a.*,b.`user_group_name`,d.`module_name`,e.`module_group_name`,module_code,c.`isenable`, a.user_guid FROM set_user a 
             ...
             WHERE a.user_id = '$userid' AND a.`user_password` = md5('$password') AND a.`isactive` = 1 
    ...
    function check_info($userid,$password)
    {
        $check = $this->db->query("SELECT * FROM lite_jr.check_user WHERE user_name = '$userid' AND user_password = '$password'");
