...
ON a.refno = b.refno  and a.customer_guid = b.customer_guid
WHERE b.`customer_guid` = '".$_SESSION['customer_guid']."' AND a.loc_group in ($loc) AND a.in_kind = 0 and (b.refno like '%$search%' or a.loc_group like '%$search%' or sup_cn_date like '%$search%' or code like '%$search%' or name like '%$search%') $q_doc_from_to $q_exp_from_to $q_refno $q_period_code");
                }
                else
                {
                    $result = $this->db->query("SELECT count(1) as count FROM b2b_summary.grmain AS a
INNER JOIN  (select * from b2b_summary.grmain_dncn WHERE customer_guid = '".$_SESSION['customer_guid']."' group by refno) AS b
ON a.refno = b.refno  and a.customer_guid = b.customer_guid
WHERE b.`customer_guid` = '".$_SESSION['customer_guid']."' AND a.loc_group in ($loc) AND a.in_kind = 0 AND ap_sup_code IN (".$_SESSION['query_supcode'].") and (b.refno like '%$search%' or a.loc_group like '%$search%' or sup_cn_date like '%$search%' or code like '%$search%' or name like '%$search%') $q_doc_from_to $q_exp_from_to $q_refno $q_period_code");
                };
        }
        ...
        if($check_module == 'panda_di')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT count(1) as count FROM b2b_summary.discheme_taxinv  WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and (inv_refno like '%$search%' or refno like '%$search%' or loc_group like '%$search%' or sup_code like '%$search%' or sup_name like '%$search%')");
                }
                else
                {
                     $result = $this->db->query("SELECT count(1) as count FROM b2b_summary.discheme_taxinv  WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].") and (inv_refno like '%$search%' or refno like '%$search%' or loc_group like '%$search%' or sup_code like '%$search%' or sup_name like '%$search%')");
                }
        }
        ...
        if($check_module == 'panda_return_collection')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT count(1) as count FROM b2b_summary.dbnote_batch AS a WHERE a.customer_guid  = '".$_SESSION['customer_guid']."' AND location IN ($loc)    $q_doc_from_to $q_exp_from_to $q_refno  and status IN ($check_status) $q_period_code and (batch_no like '%$search%' or location like '%$search%' or expiry_date like '%$search%' or a.sup_name like '%$search%' or doc_date like '%$search%' or a.sup_code like '%$search%')");
                }
                else
                {
                     $result = $this->db->query("SELECT count(1) as count FROM b2b_summary.dbnote_batch AS a WHERE a.customer_guid  = '".$_SESSION['customer_guid']."' AND location IN ($loc)    $q_doc_from_to $q_exp_from_to $q_refno and sup_code IN (".$_SESSION['query_supcode'].") and status IN ($check_status) $q_period_code and (batch_no like '%$search%' or location like '%$search%' or expiry_date like '%$search%' or a.sup_name like '%$search%' or doc_date like '%$search%' or a.sup_code like '%$search%')");
                }
        }
...
WHERE b.`customer_guid` ='".$_SESSION['customer_guid']."' and b.transtype IN ($check_status) AND a.loc_group in ($loc) AND a.in_kind = 0 $q_doc_from_to $q_exp_from_to $q_refno $q_period_code AND ap_sup_code IN (".$_SESSION['query_supcode'].") order by $order $dir  limit $start,$limit");
        ...
        if($check_module == 'panda_pdncn')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT customer_guid, trans_type, loc_group, refno, docno, docdate, CODE, name, amount, gst_tax_sum, amount_include_tax,IF(status = '', 'NEW', status) as status FROM b2b_summary.cndn_amt WHERE trans_type IN ('PCNAMT' , 'PDNAMT') AND  customer_guid = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) order by $order $dir  limit $start,$limit ");
                }
                else
                {
                     $result = $this->db->query("SELECT customer_guid, trans_type, loc_group, refno, docno, docdate, CODE, amount, gst_tax_sum, amount_include_tax,IF(status = '', 'NEW', status) as status FROM b2b_summary.cndn_amt WHERE trans_type IN ('PCNAMT' , 'PDNAMT') AND  customer_guid = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and code IN (".$_SESSION['query_supcode'].") order by $order $dir  limit $start,$limit ");
                }
        }
...
FROM b2b_summary.promo_taxinv  WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].") order by $order $dir  limit $start,$limit");
        ...
        if($check_module == 'panda_di')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT inv_refno,refno, loc_group, sup_code, sup_name, docdate, datedue, total_net,IF(status = '', 'NEW', status) as status FROM b2b_summary.discheme_taxinv WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) order by $order $dir  limit $start,$limit");
                }
                else
                {
                     $result = $this->db->query("SELECT inv_refno,refno, loc_group, sup_code, sup_name, docdate, datedue, total_net,IF(status = '', 'NEW', status) as status FROM b2b_summary.discheme_taxinv  WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].") order by $order $dir  limit $start,$limit");
                }
        }
        ...
        if($check_module == 'panda_return_collection')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT a.customer_guid, IFNULL(a.converted_by, '') AS prdn_refno, batch_no, location, doc_date, expiry_date, sup_code, sup_name, canceled, IF( STATUS = '0', 'Pending Accept', IF( STATUS = '1', 'Accepted', IF( STATUS = '2', 'Pending PRDN', IF( STATUS = '3', 'PRDN generated', IF( STATUS = '4', 'NA', IF( STATUS = '-1', 'Rejected', 'Undefined') ) ) ) ) ) AS status_desc, status, accepted_at FROM b2b_summary.dbnote_batch AS a WHERE a.customer_guid  = '".$_SESSION['customer_guid']."' AND location IN ($loc)    $q_doc_from_to $q_exp_from_to $q_refno  and status IN ($check_status) $q_period_code   order by $order $dir  limit $start,$limit");
                    // echo $this->db->last_query();die;
                }
                else
                {
                     $result = $this->db->query("SELECT a.customer_guid, IFNULL(a.converted_by, '') AS prdn_refno, batch_no, location, doc_date, expiry_date, sup_code, sup_name, canceled, IF( STATUS = '0', 'Pending Accept', IF( STATUS = '1', 'Accepted', IF( STATUS = '2', 'Pending PRDN', IF( STATUS = '3', 'PRDN generated', IF( STATUS = '4', 'NA', IF( STATUS = '-1', 'Rejected', 'Undefined') ) ) ) ) ) AS status_desc, status, accepted_at FROM b2b_summary.dbnote_batch AS a WHERE a.customer_guid  = '".$_SESSION['customer_guid']."' AND location IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].")  $q_doc_from_to $q_exp_from_to $q_refno  and status IN ($check_status) $q_period_code   order by $order $dir  limit $start,$limit");
                }
        }
...
FROM b2b_summary.promo_taxinv  WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].") and (inv_refno like '%$search%' or loc_group like '%$search%'  or sup_code like '%$search%' or sup_name like '%$search%' or  docdate like '%$search%' or promo_refno like '%$search%')  order by $order $dir  limit $start,$limit");
        ...
        if($check_module == 'panda_di')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                {
                    $result = $this->db->query("SELECT inv_refno, refno, loc_group, sup_code, sup_name, docdate, datedue, total_net,IF(status = '', 'NEW', status) as status FROM b2b_summary.discheme_taxinv WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and (inv_refno like '%$search%' or refno like '%$search%' or loc_group like '%$search%' or sup_code like '%$search%' or sup_name like '%$search%') order by $order $dir  limit $start,$limit");
                }
                else
                {
                     $result = $this->db->query("SELECT inv_refno, refno, loc_group, sup_code, sup_name, docdate, datedue, total_net,IF(status = '', 'NEW', status) as status FROM b2b_summary.discheme_taxinv WHERE customer_guid  = '".$_SESSION['customer_guid']."' AND loc_group IN ($loc) and sup_code IN (".$_SESSION['query_supcode'].") and (inv_refno like '%$search%' or refno like '%$search%' or loc_group like '%$search%' or sup_code like '%$search%' or sup_name like '%$search%')  order by $order $dir  limit $start,$limit");
                }
        }
        ...
        if($check_module == 'panda_return_collection')
        {
            if(in_array('IAVA',$_SESSION['module_code']))
                ...
                {
                     $result = $this->db->query("SELECT a.customer_guid, IFNULL(a.converted_by, '') AS prdn_refno, batch_no, location, doc_date, expiry_date, sup_code, sup_name, canceled, IF( STATUS = '0', 'Pending Accept', IF( STATUS = '1', 'Accepted', IF( STATUS = '2', 'Pending PRDN', IF( STATUS = '3', 'PRDN generated', IF( STATUS = '4', 'NA', IF( STATUS = '-1', 'Rejected', 'Undefined') ) ) ) ) ) AS status_desc, status, accepted_at FROM b2b_summary.dbnote_batch AS a WHERE a.customer_guid  = '".$_SESSION['customer_guid']."' AND location IN ($loc) and status IN ($check_status) $q_period_code and sup_code IN (".$_SESSION['query_supcode'].") and (batch_no like '%$search%' or location like '%$search%' or expiry_date like '%$search%' or a.sup_name like '%$search%' or doc_date like '%$search%' or a.sup_code like '%$search%')  order by $order $dir  limit $start,$limit");
                } 
        }
